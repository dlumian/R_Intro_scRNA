...
pbc_filter
```
You may be interested in computing new information from your data. For instance the data in the ast column (aspartate aminotransferase), is in Units/ml.  You can create a new column (called new_val here) that calculates Units/ul.
...
exp
```
A volcano plot is a specific type of scatter plot looking at the log2 fold change versus the -log10 of the adjusted p value with the points colored according to the log10 of base mean expression values.  Replace the x's with the correct variables below.
```{r}
